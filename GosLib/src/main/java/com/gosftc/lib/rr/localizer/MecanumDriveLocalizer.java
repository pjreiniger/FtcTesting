...
public class MecanumDriveLocalizer implements Localizer {
    ...
    public MecanumDriveLocalizer(
            MecanumKinematics kinematics, Encoder fl, Encoder fr, Encoder bl, Encoder br, IMU imu) {
        m_flEncoder = fl;
        m_frEncoder = fr;
        m_blEncoder = bl;
        m_brEncoder = br;
        ...
        Twist2dDual<Time> twist =
                m_kinematics.forward(
                        new MecanumKinematics.WheelIncrements<>(
                                new DualNum<Time>(
                                                new double[] {
                                                    (leftFrontPosVel.position - m_flLastPos), leftFrontPosVel.velocity,
                                                })
                                        .times(PARAMS.inPerTick),
                                new DualNum<Time>(
                                                new double[] {
                                                    (leftBackPosVel.position - m_blLastPos), leftBackPosVel.velocity,
                                                })
                                        .times(PARAMS.inPerTick),
                                new DualNum<Time>(
                                                new double[] {
                                                    (rightBackPosVel.position - m_brLastPos), rightBackPosVel.velocity,
                                                })
                                        .times(PARAMS.inPerTick),
                                new DualNum<Time>(
                                                new double[] {
                                                    (rightFrontPosVel.position - m_frLastPos), rightFrontPosVel.velocity,
                                                })
                                        .times(PARAMS.inPerTick)));
